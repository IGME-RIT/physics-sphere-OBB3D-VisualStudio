...
bool sphere_box_Collision(Sphere &s, Cuboid &c)
{
...
{
	Sphere ls;
	ls.origin = glm::vec3(0.0f, 0.0f,0.0f);
	ls.radius = s.radius;

	glm::vec3 dis = s.origin - c.origin;
	dis = glm::rotate(dis, -c.angle, c.axis);

	ls.origin = dis;

	return sphere_box_Collision(ls, c);
}
...
void setup()
{
	...
	cuboid.length = 0.5f;
	cuboid.depth = 0.5f;
	cuboid.angle = glm::radians(45.0f);
	cuboid.axis = glm::vec3(0.0f, 0.0f, 1.0f);

	//Rectangle Vertex generation

	//Rectangle Vertex generation
	glm::vec3 a, b, c, d, a2, b2, c2, d2;

	// Rotate the the 4 vertices of the rectangle before sending them to GPU

	a = glm::vec3(cuboid.origin.x - (cuboid.breadth / 2.0f),
		cuboid.origin.y - (cuboid.length / 2.0f),
		cuboid.origin.z + cuboid.depth / 2.0f);

	b = glm::vec3(cuboid.origin.x + (cuboid.breadth / 2.0f),
		cuboid.origin.y - (cuboid.length / 2.0f),
		cuboid.origin.z + cuboid.depth / 2.0f);

	c = glm::vec3(cuboid.origin.x + (cuboid.breadth / 2.0f),
		cuboid.origin.y + (cuboid.length / 2.0f),
		cuboid.origin.z + cuboid.depth / 2.0f);

	d = glm::vec3(cuboid.origin.x - (cuboid.breadth / 2.0f),
		cuboid.origin.y + (cuboid.length / 2.0f),
		cuboid.origin.z + cuboid.depth / 2.0f);

	a2 = glm::vec3(cuboid.origin.x - (cuboid.breadth / 2.0f),
		cuboid.origin.y - (cuboid.length / 2.0f),
		cuboid.origin.z - cuboid.depth / 2.0f);

	b2 = glm::vec3(cuboid.origin.x + (cuboid.breadth / 2.0f),
		cuboid.origin.y - (cuboid.length / 2.0f),
		cuboid.origin.z - cuboid.depth / 2.0f);

	c2 = glm::vec3(cuboid.origin.x + (cuboid.breadth / 2.0f),
		cuboid.origin.y + (cuboid.length / 2.0f),
		cuboid.origin.z - cuboid.depth / 2.0f);

	d2 = glm::vec3(cuboid.origin.x - (cuboid.breadth / 2.0f),
		cuboid.origin.y + (cuboid.length / 2.0f),
		cuboid.origin.z - cuboid.depth / 2.0f);

	a = glm::rotate(a, cuboid.angle, cuboid.axis);
	b = glm::rotate(b, cuboid.angle, cuboid.axis);
	c = glm::rotate(c, cuboid.angle, cuboid.axis);
	d = glm::rotate(d, cuboid.angle, cuboid.axis);
	a2 = glm::rotate(a2, cuboid.angle, cuboid.axis);
	b2 = glm::rotate(b2, cuboid.angle, cuboid.axis);
	c2 = glm::rotate(c2, cuboid.angle, cuboid.axis);
	d2 = glm::rotate(d2, cuboid.angle, cuboid.axis);
	...
	VertexFormat A2 = VertexFormat(a2, glm::vec4(0.7f, 0.20f, 0.0f, 1.0f));

	VertexFormat B2 = VertexFormat(b2, glm::vec4(0.7f, 0.20f, 0.0f, 1.0f));

	VertexFormat C2 = VertexFormat(c2, glm::vec4(0.7f, 0.20f, 0.0f, 1.0f));

	VertexFormat D2 = VertexFormat(d2, glm::vec4(0.7f, 0.20f, 0.0f, 1.0f));
...
void renderScene()
{
	...
	//Draw the sphere
	// Set the uniform matrix in our shader to our MVP matrix for the Sphere.
...
void main()
{
	...
	window = glfwCreateWindow(800, 800, "Sphere-OBB Collision in 3 Dimesions ", nullptr, nullptr);

	std::cout << "\n This is a collision test between a sphere \n and a oriented bounding box in 3D.\n\n\n\n\n";
